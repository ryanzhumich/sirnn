...
class Sample(object):
    ...
    def __init__(self, context, spk_id, adr_id, responses, label, n_agents_in_ctx, max_n_agents):
        ...
        self.adr_agents = [s.index(1) if 1 in s else -1 for s in self.adr_agent_one_hot_vec]

        self.true_res = label
        self.true_adr = get_adr_label(adr_id, self.agent_index_dict)

        self.n_agents_in_lctx = len(set([c[1] for c in context] + [spk_id]))
        self.binned_n_agents_in_lctx = bin_n_agents_in_lctx(self.n_agents_in_lctx)

        self.binned_n_agents_in_ctx = bin_n_agents_in_ctx(n_agents_in_ctx)
        self.n_agents_in_ctx = n_agents_in_ctx

        if self.true_adr == -1:
            self.adr_distance = self.true_adr
        else:
            self.adr_distance = list(reversed(self.spk_agents)).index(self.true_adr+1)
        self.binned_n_agents_in_distance = bin_n_agents_in_distance(self.adr_distance)
...
def bin_n_agents_in_lctx(n):
    return n

def bin_n_agents_in_distance(n):
    #if n == -1:
    #    return -1
    return n + 1
